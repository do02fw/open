...
sed -i 's/1.0.0.1/2606:4700:4700::1111/g' /etc/openvpn/server/server.conf && echo '--data-ciphers CHACHA20-POLY1305' >> /etc/openvpn/server/server.conf && echo 'duplicate-cn' >> /etc/openvpn/server/server.conf && echo 'mtu-disc maybe' >> /etc/openvpn/server/server.conf
